...
charsToNums :: Map.Map Char Int
numsToChars :: Map.Map Int Char
...
-- Take in a string which hopefully represents a base 64 (or whatever) value, and attempt to convert it to an appropriate Int
-- If the string contains invalid characters, Nothing is returned
decodeL :: String -> Maybe Int
decodeL = fmap (foldl appendNum 0) . mapM (flip Map.lookup charsToNums)
     where appendNum acc n = (acc * base) + n

-- Take in a string which hopefully represents a base 64 (or whatever) value, and attempt to convert it to an appropriate Int
-- If the string contains invalid characters, Nothing is returned
decodeR :: String -> Maybe Int
